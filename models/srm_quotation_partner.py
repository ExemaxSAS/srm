...
class Opportunity2Quotation(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if active_model != 'srm.lead':
            raise UserError(_('You can only apply this action from a lead.'))

        lead = False
        if result.get('lead_id'):
            lead = self.env['srm.lead'].browse(result['lead_id'])
        elif 'lead_id' in fields and self._context.get('active_id'):
            lead = self.env['srm.lead'].browse(self._context['active_id'])
        if lead:
    ...
    lead_id = fields.Many2one('srm.lead', "Associated Lead", required=True)
